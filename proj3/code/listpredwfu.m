...
% update the features and see what would happen... 

  feat_train = ftupdate(feat_train, feat_train(sub2ind(size(scores), indices, 1: size(scores, 2)), :)); 
...
% the indices that yield the minimum scores will be extracted, and their labels will be saved.  
  ...
  feat_test = ftupdate(feat_test, feat_test(sub2ind(size(scores), indices, 1: size(scores, 2)), :)); 
...
legend('List Prediction with Feature Update')
